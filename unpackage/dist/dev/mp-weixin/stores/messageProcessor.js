...
const useMessageProcessorStore = common_vendor.defineStore("messageProcessor", () => {
  ...
  const processMessage = (data) => {
    if (typeof data === "string") {
      try {
        ...
        common_vendor.index.__f__("error", "at stores/messageProcessor.js:25", "消息解析失败", e);
    ...
    switch (data.cmd) {
      ...
      case "error":
        ...
        break;
      default:
        common_vendor.index.__f__("warn", "at stores/messageProcessor.js:51", "未知消息类型", data);
  ...
  const handleBgMusic = (data) => {
    common_vendor.index.__f__("log", "at stores/messageProcessor.js:60", "收到背景音乐消息", data);
    ...
    if (data.is_radio && data.is_radio == 1) {
      common_vendor.index.__f__("log", "at stores/messageProcessor.js:68", "当前是广播模式");
      isRadioStore.setIsRadio(true);
      audioPlayerStore.setBgLoop(false);
    }
    if (data.is_radio && data.lrc) {
      common_vendor.index.__f__("log", "at stores/messageProcessor.js:73", "当前是广播模式,歌词:", data.lrc);
    }
  };
  const handleTtsAudio = (data) => {
    common_vendor.index.__f__("log", "at stores/messageProcessor.js:86", "收到TTS音频消息", data);
    if (data.is_radio && data.is_radio == 1) {
      common_vendor.index.__f__("log", "at stores/messageProcessor.js:88", "当前是广播模式");
      isRadioStore.setIsRadio(true);
    }
    ...
    if (text) {
      common_vendor.index.__f__("log", "at stores/messageProcessor.js:100", "处理文本:");
      if (!isStreaming.value || lastSectionId.value !== section_id) {
        common_vendor.index.__f__("log", "at stores/messageProcessor.js:104", "开始新的流式消息", {
      ...
      common_vendor.index.__f__("log", "at stores/messageProcessor.js:120", "当前累积文本:", accumulatedText.value);
    }
  };
  const handleFinishMessage = (data) => {
    common_vendor.index.__f__("log", "at stores/messageProcessor.js:126", "收到结束消息", data);
  ...
  const handleSubjectRequest = (data) => {
    common_vendor.index.__f__("log", "at stores/messageProcessor.js:146", "收到主题选择请求", data);
    const subjects = data.subjects.join("\n");
    common_vendor.index.__f__("log", "at stores/messageProcessor.js:148", "可选主题列表", subjects);
  ...
  const handleErrorMessage = (data) => {
    common_vendor.index.__f__("log", "at stores/messageProcessor.js:158", "收到错误消息", data);
    const { text } = data;
