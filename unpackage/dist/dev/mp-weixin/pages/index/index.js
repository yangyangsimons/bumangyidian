...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    common_vendor.computed(() => isRadioStore.isRadio);
    ...
    const toggleSystemModel = async () => {
      ...
      if (isTogglingModel.value) {
        common_vendor.index.__f__("log", "at pages/index/index.vue:194", "切换模式操作进行中，请勿重复点击");
        common_vendor.index.showToast({
          title: "操作进行中，请稍候",
          icon: "none"
        });
      ...
      try {
        ...
        common_vendor.index.__f__("log", "at pages/index/index.vue:210", `切换到${currentModel.value}`);
        audioPlayerStore.reportCurrentProgress();
        audioPlayerStore.stopAllAudio();
        common_vendor.index.__f__("log", "at pages/index/index.vue:215", "停止并清空所有音频队列");
        barrageStore.clearMessages();
        common_vendor.index.__f__("log", "at pages/index/index.vue:217", "清空消息列表");
        ...
        await wsStore.close();
        common_vendor.index.__f__("log", "at pages/index/index.vue:228", "WebSocket连接已关闭");
        await new Promise((resolve) => setTimeout(resolve, 1500));
        try {
          await wsStore.connect();
          common_vendor.index.__f__("log", "at pages/index/index.vue:236", "socket重新连接成功");
          ...
          if (currentModel.value === "金种子杯模式") {
            ...
            common_vendor.index.__f__("log", "at pages/index/index.vue:248", "模式切换消息发送成功");
          } else {
            modelStore.setModel("常规模式");
            await wsStore.sendMessage({
              system_model: currentModel.value,
            ...
            common_vendor.index.__f__("log", "at pages/index/index.vue:256", "模式切换消息发送成功");
          }
          common_vendor.index.showToast({
            title: "模式切换成功",
            icon: "success",
          ...
          common_vendor.index.__f__("error", "at pages/index/index.vue:265", "WebSocket操作失败:", error);
          common_vendor.index.showToast({
            title: "切换失败，请稍后再试",
            icon: "none"
          });
        ...
        common_vendor.index.__f__("error", "at pages/index/index.vue:281", "切换模式过程中出错:", error);
        common_vendor.index.showToast({
          title: "切换失败，请稍后再试",
          icon: "none"
        });
    ...
    const handleSubmit = (message) => {
      common_vendor.index.__f__("log", "at pages/index/index.vue:292", message, "handleSubmit");
    };
    const fetchSystemConfig = async () => {
      try {
        const res = await utils_request.request(`${utils_config.baseUrl}/system/get_system_setting`, "GET");
        common_vendor.index.__f__("log", "at pages/index/index.vue:299", "获取系统配置", res);
        ...
        common_vendor.index.__f__("error", "at pages/index/index.vue:326", "获取系统配置失败:", error);
      }
    };
    common_vendor.onLoad(async () => {
      try {
        common_vendor.index.__f__("log", "at pages/index/index.vue:333", "主页面显示");
        const currentSubject = await utils_request.request(`${utils_config.baseUrl}/user/user_info`, "GET");
        common_vendor.index.__f__("log", "at pages/index/index.vue:336", "获取当前主题", currentSubject.data.topic);
        sbStore.setSubject(currentSubject.data.topic);
        await fetchSystemConfig();
        if (!wsStore.isConnected) {
          await wsStore.connect();
          common_vendor.index.__f__("log", "at pages/index/index.vue:345", "socket连接成功");
          await wsStore.sendMessage({
            system_model: currentModel.value,
            input_type: 3,
            text: ""
          });
          common_vendor.index.__f__("log", "at pages/index/index.vue:353", "发送input_type=3的初始消息成功");
        }
      } catch (error) {
        common_vendor.index.__f__("error", "at pages/index/index.vue:356", "页面显示时发生错误:", error);
        common_vendor.index.showToast({
          title: "连接失败，请重试",
          icon: "none"
        });
    ...
    common_vendor.onUnload(() => {
      common_vendor.index.__f__("log", "at pages/index/index.vue:366", "onHide主页面隐藏");
      audioPlayerStore.reportCurrentProgress();
      common_vendor.index.__f__("log", "at pages/index/index.vue:370", "音频播放状态已上报");
      audioPlayerStore.stopAllAudio();
      common_vendor.index.__f__("log", "at pages/index/index.vue:374", "停止并清空所有音频队列");
      barrageStore.clearMessages();
      common_vendor.index.__f__("log", "at pages/index/index.vue:377", "清空消息列表");
      wsStore.close();
      common_vendor.index.__f__("log", "at pages/index/index.vue:381", "socket连接关闭");
    });
    return (_ctx, _cache) => {
