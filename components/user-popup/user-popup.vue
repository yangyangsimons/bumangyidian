...
<script setup>
  ...
  onShow(async () => {
    // 页面显示时，获取用户信息
    try {
      const toneRes = await uni.request({
        url: `${baseUrl}/tone/query`,
        method: 'get',
      })
      console.log('获取音色信息', toneRes)
      if (toneRes.data.code === 0) {
        console.log('获取音色信息成功', toneRes.data.data)
        tones.value = toneRes.data.data
        // 处理音色信息
        // 例如：将音色信息存储到状态管理中
      } else {
        console.error('获取音色信息失败', toneRes.data.message)
      }
